<template>
  <div>
    ...
    >
      ...
      <el-table-column
        v-if="has('sys_request_log_detail')"
...
export default {
    ...
    return {
      ...
      formConfig: {
        ...
        items: [
          ...
          { type: 'input', prop: 'code', label: '响应状态码' },
      ...
      tableConfig: {
        ...
        columns: [
          ...
          { prop: 'code', label: 'code', width: 80 },
          { prop: 'username', label: '用户名', width: 120 },
          { prop: 'name', label: '用户姓名', width: 80 },
          {
            prop: 'startTime',
            label: '请求时间',
            width: 170,
  ...
  methods: {
    ...
    fetchTableData() {
      ...
      this.$http
        .post('/auth/sys/log/view/request/findPage', {
    ...
    setBeginAndEndTime(arr) {
      ...
      let beginDate = hasDate ? this.$utils.formatDate(new Date(arr[0]), 'yyyy/MM/dd') : ''
      let endDate = hasDate ? this.$utils.formatDate(new Date(arr[1]), 'yyyy/MM/dd') : ''
    ...
    excelExport() {
      this.loading = true
      this.$http
        .postResponseBlob('/auth/sys/log/op/request/export', this.formData)
        .then(blob => {
          const time = this.$utils.formatDate(new Date(), 'yyyy-MM-dd_HH:mm')
          this.$utils.downloadBlob(blob, '请求日志列表_' + time + '.xlsx')
        })
        .finally(() => {
          this.loading = false
        })
